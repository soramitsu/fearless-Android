...
class ChainSyncService(
    ...
    private suspend fun configChainsSyncUp(): List<Chain> = supervisorScope {
        ...
        logger("remote chains fetched: ${remoteChains.size}")
        remoteChains.take(5).forEach { logger("remote chain id: ${it.id}") }
        ...
        logger("local chains after sync: ${postLocal.size}")
        postLocal.take(5).forEach { logger("local chain id: ${it.chain.id}") }
