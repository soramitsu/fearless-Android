...
class OptionsEcosystemAccountsViewModel @Inject constructor(
    ...
    val type = savedStateHandle.get<WalletEcosystem>(OptionsEcosystemAccountsFragment.KEY_TYPE) ?: error("Account type not specified")
        ...
        launch {
            accountInteractor.getBestBackupType(walletId, type)?.let {
                val usedChainId = when (type) {
                    ...
                    WalletEcosystem.Ton -> tonMainnetChainId
                }
                when (it) {
