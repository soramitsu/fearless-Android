...
class PayoutRepository(
    ...
    suspend fun calculateUnpaidPayouts(stashAddress: String): List<ValidatorHistoricalStats> {
        val validatorAddresses = validatorSetFetcher.fetchAllValidators(stashAddress)

        return getValidatorHistoricalStats(validatorAddresses)
    }

    private suspend fun getValidatorHistoricalStats(validatorAddresses: List<String>): List<ValidatorHistoricalStats> {
        ...
        return validatorAddresses.mapNotNull { validatorAddress ->
            ...
            ValidatorHistoricalStats(ledger, historicalStats)
        }
    }
}
