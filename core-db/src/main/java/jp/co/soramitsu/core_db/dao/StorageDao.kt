...
abstract class StorageDao {
    ...
    @Query("SELECT * from storage WHERE networkType = :networkType AND storageKey in (:fullKeys)")
    abstract fun observeEntries(networkType: Node.NetworkType, fullKeys: List<String>): Flow<List<StorageEntryLocal>>
