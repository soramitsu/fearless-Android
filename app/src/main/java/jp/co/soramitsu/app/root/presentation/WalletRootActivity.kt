...
@AndroidEntryPoint
class WalletRootActivity : BaseActivity<RootViewModel>(), LifecycleObserver {

    companion object {
        private const val ANIM_DURATION = 150L
        private const val ANIM_START_POSITION = 100f
        private const val NETWORK_PROBE_TIMEOUT_MILLIS = 1_000
    }
    ...
    private val secureStorageRestartRedirector =
        WalletSecureStorageRestartRedirector(
            ...
            canRedirectNow = {
                contentInitializationAllowed &&
                    secureStorageRedirectHostStarted &&
                    !isFinishing &&
                    !isDestroyed
            },
            redirectToStartup = {
                contentInitializationAllowed = false
    ...
    override fun canInitializeContent(): Boolean {
        return WalletStartupSession.isReady().also { allowed ->
            contentInitializationAllowed = allowed
        }
    }

    override fun onContentInitializationBlocked() {
        val startupIntent = intent.toWalletInternalHandoff(
            context = this,
            target = WalletStartupActivity::class.java
        )
    ...
    override fun onRestoreInstanceState(savedInstanceState: Bundle) {
        if (!contentInitializationAllowed) return
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        secureStorageRestartRedirector.register()
    ...
    override fun onStart() {
        super.onStart()
        secureStorageRedirectHostStarted = true
        secureStorageRestartRedirector.onHostStarted()
    }

    override fun onStop() {
        secureStorageRedirectHostStarted = false
        super.onStop()
    }
    ...
    private fun subscribeNetworkStatus() {
        ...
        lifecycleScope.launch {
            while (isActive) {
                ...
                val canReachChains = isConnected || hasInternetAccess()
    ...
    private suspend fun hasInternetAccess(): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                urlConnection.connectTimeout = NETWORK_PROBE_TIMEOUT_MILLIS
                urlConnection.readTimeout = NETWORK_PROBE_TIMEOUT_MILLIS
    ...
    override fun onDestroy() {
        secureStorageRedirectHostStarted = false
        secureStorageRestartRedirector.unregister()
        animation?.cancel()
        animation = null
    ...
    private fun unregisterNetworkCallback() {
        ...
        val networkCallback = registeredNetworkCallback
        ...
        try {
            ...
            Timber.w(
                failure,
                "Network callback was already unregistered during teardown"
            )
        }
    }
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        // Keep the Activity's retained Intent aligned with the newest handoff.
        // If secure storage is invalidated later, the restart redirect must
        // forward this payload instead of replaying the original launch Intent.
        setIntent(intent)
        if (!contentInitializationAllowed) return
