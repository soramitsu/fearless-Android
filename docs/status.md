...
- Schema 79 adds `portable_wallet_reservations` and a database insert trigger that rejects a visible `meta_accounts` row using a reserved receive ID. The internal fresh-install stager commits exact Room reservations with the encrypted `FPWCJ001` journal under the wallet mutation mutex. A crash after the encrypted commit but before the Room commit can be reconciled only when the journal, reservation markers, target namespaces and empty wallet database still agree. Mismatched or orphaned Room reservations stay fenced and require explicit quarantine; exact-token abandonment removes both stores, and a failed Room commit leaves the Room fence in place. New wallet rows, V1/V2/V3 target secrets and feature enablement remain untouched.
- The v78→79 migration has an exported Room schema and an instrumentation test for preservation and explicit/automatic ID collision rejection. Local account JVM tests pass 295/295; core-db JVM tests report 29 passed, four existing skips; both Android-test APKs compile. Default and forced scoped Detekt pass. No device is attached here, so the migration trigger and signed-upgrade behavior still require hosted API 30/31/36 and distribution testing. The release migration-result verifier and fixtures now target exact schema-79 identities (2 positive/34 adversarial verifier cases; 49 evidence-packaging tests pass).

## Portable cohort fresh-install reservation prerequisite — 2026-09-24

- A new internal fresh-install stager checks that Room has no wallets, allocates distinct positive local IDs under the existing cross-store wallet mutex, and durably stores the exact `FPWCAI01` cohort journal with encrypted preference-side ID markers in one compare-and-swap. The normal signed-wallet allocator skips marked IDs; the watch-only allocator rejects a marked inserted ID and rolls its Room transaction back. An active cohort journal blocks both creators even if a marker disappears. A restart can reload the exact journal and its markers; exact-token abandonment removes both together. Orphan or changed markers fail closed and need explicit reconciliation.
