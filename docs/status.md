...
- Utils composite build: `settings.gradle` now shims the removed `jcenter()` repository helper and re-adds the deprecated `JavaExec.main` property so Gradle 9+ can still include the local `fearless-utils-Android` checkout (ktlint tasks) until it is updated upstream.
