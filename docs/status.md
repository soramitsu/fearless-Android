...
- The two existing Android wallet display preferences now have bounded shared-format metadata IDs: `10` for `wallet_selected_chain_id<id>` and `11` for `chain_select_filter_applied_<id>`. Each optional TLV contains the exact UTF-8 value, including a zero-length value when a preference is explicitly present but empty. They are captured under the cross-store lock for both signed and watch wallets and re-read after source/signing proof; missing values, malformed text, overlong text or a changed preference fail closed. The source-cohort proof accepts only these mapped Android metadata IDs. Explicit asset-row presentation settings remain blocked until mapped. The cross-platform installer/readback, canonical identity digests, live reviewed genesis policy, device validation and backup-complete wiring remain open. Recovery remains disabled.
- Final-source account JVM tests pass 328/328 with no failures or skips, including mixed historical-source coverage and the 70-byte cross-platform watch-wallet metadata vector (SHA-256 `e8959e6aa11fd339fd92ddd65798beda0b6443fe1d1be60f6ad219e2d26c3477`). Default `detektAll`, forced scoped Detekt over the five guard/codec/proof/exporter-test files, and the exact pinned Utils/WebSocket source-tree verifier pass. Forced Detekt over all seven touched Kotlin files reports the same 32 issues as the parent source, with zero introduced findings; the longstanding preflight/test formatting and size debt is not waived as a release gate. These are local source checks, not replacement-device or delivered-build acceptance.
