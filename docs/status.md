...
- This is a production recovery component, not a complete backup. Known wallet-specific chain selection/filter and explicit asset preference rows now block capture instead of being silently omitted. Their complete cross-platform mapping, replacement-device installation/readback, canonical identity digests, live reviewed genesis policy, device validation and backup-complete wiring remain open. The source-cohort proof deliberately rejects semantic metadata until its authoritative source mapping is implemented. Recovery remains disabled.
- Final-source account JVM tests pass 322/322 with no failures or skips, including mixed historical-source and global orphan/recovery-alias exporter coverage. Default `detektAll`, forced scoped Detekt for the changed guard and tests, and the exact pinned Utils/WebSocket source-tree verifier pass. These are local source checks, not replacement-device or delivered-build acceptance.
