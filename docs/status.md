...
- The account preflight now has a separate read-only `captureVerifiedSemanticPlaintext` entry point. It captures fresh caller-owned `FPWMSM01` bytes from current Room wallet, chain and favorite rows plus guarded V1/V2/V3 encrypted secret stores under the cross-store mutation lock. It keeps selected wallet/order, watch custody and standalone EVM/native TON slots, then requires the combined source/signing proof and an exact caller-supplied approved-genesis policy before returning plaintext. An exact wallet-scoped encrypted-key-name inventory rejects missing, quarantined, orphaned or unsupported active source namespaces, including leftover V2 root material. Known pre-V1 private/seed aliases and quarantined legacy sources also block capture. It resamples public identities, V1 ownership, custody, recovery state and source-key names after proof. Failure clears the encoded buffer; the verified entry point never inserts custody markers, writes a backup state or uploads bytes.
- This is a production recovery component, not a complete backup. Known wallet-specific chain selection/filter and explicit asset preference rows now block capture instead of being silently omitted. Their complete cross-platform mapping, globally orphaned namespaces, replacement-device installation/readback, canonical identity digests, live reviewed genesis policy, device validation and backup-complete wiring remain open. The source-cohort proof deliberately rejects semantic metadata until its authoritative source mapping is implemented. Recovery remains disabled.
- Final-source account JVM tests pass 320/320 with no failures or skips, including mixed historical-source and failure-path exporter coverage. Default `detektAll`, forced scoped Detekt for the new guard and tests, and the exact pinned Utils/WebSocket source-tree verifier pass. These are local source checks, not replacement-device or delivered-build acceptance.
