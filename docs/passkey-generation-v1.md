...
Retain prior accepted and unresolved generations. There is no automatic deletion or final-credential removal in this increment. Rotating a DEK requires verified surviving credentials and cannot invalidate a DEK already learned by an old credential holder. Real head/grant HTTP integration, owner bootstrap, native round-trip and enrollment/revocation coordination, same-project Google configuration/consent, provider/device acceptance and cross-platform restore remain incomplete. All release flags remain off.
