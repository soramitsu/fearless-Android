...
- 2026-09-24 wallet-material preflight: the Android candidate now fails closed when any persisted wallet's validated V3 root or V2 chain material is unavailable, or when public wallet metadata changes during the read. P0: agree the cross-platform plaintext schema, capture all secrets under a coherent wallet mutation lease, represent V1 and watch-only cohorts explicitly, implement iOS/Android serialization and replacement-device installation, and prove original-key signing/export. The preflight is not wired to backup completion and cannot make passkey recovery eligible by itself.
