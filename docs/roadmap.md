...
- 2026-09-24 wallet-material capture: the Android candidate now has an internal bounded draft serializer for every validated V3 Substrate/EVM/TON root and V2 chain key across multiple wallets, with public identity, selection/order and favorites. It captures under the existing mutation lock and rechecks durable identities; V1-only and watch-only cohorts fail closed. P0: agree one semantic and binary iOS/Android plaintext contract, migrate remaining historical material, implement atomic replacement-device installation and verify original-key signing/export. This Android-only draft is not wired to Drive or backup completion and cannot make passkey recovery eligible by itself.
